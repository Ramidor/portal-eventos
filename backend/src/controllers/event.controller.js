...
exports.create = async (req, res) => {
  try {
    ...
    if (eventDate <= new Date()) {
      return res
        .status(400)
        .json({ error: "La fecha del evento debe ser futura" });
    }
    ...
    if (
      ...
      maxAttendees !== null &&
      Number(maxAttendees) < 1
    ) {
      return res
        .status(400)
        .json({ error: "El límite de asistentes debe ser al menos 1" });
    }
...
exports.update = async (req, res) => {
  try {
    ...
    if (title && title !== event.title)
      changes.push({ field: "Título", value: title });
    if (date && new Date(date).getTime() !== event.date.getTime())
      changes.push({
        field: "Fecha",
        value: new Date(date).toLocaleString("es-ES"),
      });
    if (location && location !== event.location)
      changes.push({ field: "Lugar", value: location });
    ...
    if (changes.length > 0 && event._count.enrollments > 0) {
      getEnrolleeEmails(id)
        .then((emails) => {
          emails.forEach((email) =>
            sendEventModified(email, updated.title, changes).catch(
              console.error,
            ),
          );
        })
...
exports.remove = async (req, res) => {
  try {
    ...
    const enrolleeEmails =
      event._count.enrollments > 0 ? await getEnrolleeEmails(id) : [];
    ...
    enrolleeEmails.forEach((email) =>
      sendEventCancelledToEnrollee(email, event.title, event.date).catch(
        console.error,
      ),
    );
    if (isAdminDelete) {
      sendEventCancelledToCreator(event.creator.email, event.title).catch(
        console.error,
      );
    }
  } catch (error) {
