...
exports.submitRating = async (req, res) => {
  try {
    ...
    if (!score || score < 1 || score > 5 || !Number.isInteger(Number(score))) {
      return res
        .status(400)
        .json({ error: "La puntuación debe ser un entero entre 1 y 5" });
    }
    ...
    if (new Date() <= event.date) {
      return res
        .status(400)
        .json({ error: "Solo puedes valorar eventos que ya han terminado" });
    }
    if (event.creatorId === raterId) {
      return res
        .status(400)
        .json({ error: "No puedes valorar tu propio evento" });
    }
    ...
    if (!enrolled) {
      return res
        .status(403)
        .json({ error: "Solo los asistentes inscritos pueden valorar" });
    }
