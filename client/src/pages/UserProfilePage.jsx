  ...
  return (
    <div className="min-h-screen bg-zinc-950">
      ...
      <main className="max-w-4xl mx-auto px-6 py-12">
        ...
        {/* Cabecera */}
        <div className="mb-10">
          <p className="text-zinc-500 font-mono text-xs tracking-widest uppercase mb-2">Organizador</p>
          <div className="flex items-center gap-4">
            ...
            {ratingSummary.average !== null && (
              <div className="flex items-center gap-2 bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-2">
                ...
                <span className="text-zinc-100 font-mono font-semibold">{ratingSummary.average}</span>
                <span className="text-zinc-500 text-sm">({ratingSummary.total} valoraciones)</span>
              </div>
            )}
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {events.length === 0 && (
              <p className="text-zinc-600 font-mono text-sm">Todavía no ha organizado ningún evento.</p>
            )}
          </div>
          ...
          <div className="space-y-4">
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
              <h2 className="text-zinc-400 font-mono text-xs tracking-widest uppercase mb-4">Valoraciones</h2>

              {ratings.length === 0 ? (
                <p className="text-zinc-600 text-sm">Sin valoraciones todavía.</p>
              ) : (
                <div className="space-y-4">
                  {ratings.map((r) => (
                    <div key={r.id} className="border-t border-zinc-800 pt-4 first:border-0 first:pt-0">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-zinc-300 text-sm font-medium">{r.rater.name}</span>
                        <span className="text-orange-400 font-mono text-sm">{"★".repeat(r.score)}{"☆".repeat(5 - r.score)}</span>
                      </div>
                      {r.comment && (
...
function EventRow({ event }) {
  ...
  return (
    ...
    >
      <div>
        <p className={`text-sm font-medium ${isPast ? "text-zinc-500" : "text-zinc-200"}`}>{event.title}</p>
        <p className="text-zinc-500 text-xs font-mono mt-0.5">
          {(() => { const I = CATEGORY_ICONS[event.category] || Tag; return <><I size={11} className="inline align-middle mr-0.5" />{CATEGORY_LABELS[event.category] || "Otro"}</>; })()} · {formatted}
        </p>
      </div>
