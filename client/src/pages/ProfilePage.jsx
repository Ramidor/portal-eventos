    ...
    Promise.allSettled([
      ...
      .then(([eventsRes, enrollRes, ratingRes]) => {
        if (eventsRes.status === "fulfilled")
          setMyEvents(eventsRes.value.data.events);
        if (enrollRes.status === "fulfilled")
          setMyEnrollments(enrollRes.value.data.enrollments);
        if (ratingRes.status === "fulfilled")
          setRatingSummary(ratingRes.value.data);
      })
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
            ...
            {!editing ? (
              ...
              <form onSubmit={handleSave} className="space-y-4">
                <div>
                  <label className="block text-zinc-500 text-xs font-mono mb-1">
                    Nombre
                  </label>
                  <input
                    type="text"
                ...
                <div className="pt-2 border-t border-zinc-700">
                  ...
                  <div className="space-y-3">
                    <div>
                      <label className="block text-zinc-600 text-xs font-mono mb-1">
                        Contraseña actual
                      </label>
                      <input
                        type="password"
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
            ...
            {loading ? (
              <p className="text-zinc-600 text-sm animate-pulse">Cargando...</p>
            ) : myEvents.length === 0 ? (
              <p className="text-zinc-600 text-sm">
                Todavía no has organizado ningún evento.
              </p>
            ) : (
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
            ...
            {loading ? (
              ...
              myEnrollments.map((e) => (
                <EventRow key={e.event.id} event={e.event} />
              ))
            )}
          </div>
        </div>
