...
export default function EditEventPage() {
  ...
  const handleSubmit = async (form) => {
    ...
    try {
      ...
      setSubmitError(
        err.response?.data?.error || "Error al actualizar el evento",
      );
    } finally {
      setLoading(false);
    }
  ...
  if (fetchError)
    return (
      <PageLayout maxWidth="max-w-2xl">
        <p className="text-red-400 font-mono text-sm text-center py-20">
          {fetchError}
        </p>
      </PageLayout>
    );
  ...
  return (
    <PageLayout maxWidth="max-w-2xl">
      <div className="mb-10">
        ...
        <h1 className="text-4xl font-serif text-zinc-100 line-clamp-2">
          {event.title}
        </h1>
      </div>
