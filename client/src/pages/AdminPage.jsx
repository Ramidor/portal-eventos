...
export default function AdminPage() {
  ...
  return (
    <PageLayout maxWidth="max-w-5xl">
        ...
        {loading && <p className="text-zinc-500 font-mono text-sm animate-pulse">Cargando...</p>}
        {error && (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-800">
                  <th className="text-left text-zinc-500 font-mono text-xs tracking-widest uppercase px-6 py-4">Usuario</th>
                  <th className="text-left text-zinc-500 font-mono text-xs tracking-widest uppercase px-6 py-4">Rol</th>
                  <th className="text-left text-zinc-500 font-mono text-xs tracking-widest uppercase px-6 py-4 hidden md:table-cell">Eventos</th>
                  <th className="text-left text-zinc-500 font-mono text-xs tracking-widest uppercase px-6 py-4 hidden md:table-cell">Inscripciones</th>
                  ...
                  <tr key={u.id} className="border-b border-zinc-800/50 last:border-0 hover:bg-zinc-800/30 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`text-xs font-mono px-2 py-1 rounded-full ${
                        u.role === "ADMIN" ? "bg-orange-400/10 text-orange-400" : "bg-zinc-800 text-zinc-400"
                      }`}>{u.role}</span>
                    </td>
                    <td className="px-6 py-4 text-zinc-400 hidden md:table-cell">{u._count?.organizedEvents ?? 0}</td>
                    <td className="px-6 py-4 text-zinc-400 hidden md:table-cell">{u._count?.enrollments ?? 0}</td>
                    <td className="px-6 py-4">
                      {u.id !== user.id ? (
                        ...
                        <span className="text-zinc-700 font-mono text-xs">Tú</span>
                      )}
                    </td>
                  </tr>
                ))}
        ...
        {!loading && !error && tab === "events" && (
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-800">
                  <th className="text-left text-zinc-500 font-mono text-xs tracking-widest uppercase px-6 py-4">Evento</th>
                  <th className="text-left text-zinc-500 font-mono text-xs tracking-widest uppercase px-6 py-4 hidden md:table-cell">Categoría</th>
                  <th className="text-left text-zinc-500 font-mono text-xs tracking-widest uppercase px-6 py-4 hidden md:table-cell">Inscritos</th>
                  <th className="px-6 py-4" />
                </tr>
              </thead>
              <tbody>
                {events.map((e) => {
                  ...
                  return (
                    <tr key={e.id} className="border-b border-zinc-800/50 last:border-0 hover:bg-zinc-800/30 transition-colors">
                      <td className="px-6 py-4">
                        <p className={`font-medium ${isPast ? "text-zinc-500" : "text-zinc-200"}`}>{e.title}</p>
                      ...
                      <td className="px-6 py-4 text-zinc-400 hidden md:table-cell text-xs font-mono">
                        {(() => { const I = CATEGORY_ICONS[e.category] || Tag; return <span className="inline-flex items-center gap-1"><I size={11} />{CATEGORY_LABELS[e.category] || "Otro"}</span>; })()}
                      </td>
                      <td className="px-6 py-4 text-zinc-400 hidden md:table-cell">
                        {e._count?.enrollments ?? 0}
                        {e.maxAttendees && (
                          <span className="text-zinc-600"> / {e.maxAttendees}</span>
                        )}
                      </td>
            ...
            {events.length === 0 && (
              <p className="text-zinc-600 text-sm font-mono text-center py-10">No hay eventos.</p>
            )}
          </div>
        )}
